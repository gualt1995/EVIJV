...
namespace AIMLbot.Utils 
{
	...
	public class AIMLLoader
    {
		...
		// Windows, Linux and Max OSX plattform
		...
		/// <summary>
		/// Loads the AIML file from resources.
		/// </summary>
		/// <returns>The AIML file from resources.</returns>
		void LoadAIMLFileFromResources(string _filename)
		{
			this.bot.writeToLog("Load AIML file from resources: " + _filename);
			...
			if (_filename.Length > 0)
			{
				...
				_textAsset = (TextAsset) Resources.Load(_filename,typeof(TextAsset));
				...
				if (_textAsset!=null)
				{
					...
					this.loadAIMLFromXML(_xmlDoc, _filename);
				}
				else
				{
...
// Android and IOS plattform
			...
			if (AIMLFiles.Count > 0)
			{
				...
				foreach (string filename in AIMLFiles)
				{
					...
					string _filenameWithoutExtension = filename.Remove(filename.LastIndexOf("."));
					// Calls aiml load function
					LoadAIMLFileFromResources(path + "/" + _filenameWithoutExtension);
				}
